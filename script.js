...
getMovieBtn.addEventListener("click", () => {
  ...
  axios
    ...
    .then((response) => {
      ...
      const genreLength = movieData.genres.length;
      let gnrNames = "";
      for (const gnr of movieData.genres) {
        gnrNames = gnrNames + gnr.name + ", ";
      }
      // removing extra coma and space from end
      gnrNames = gnrNames.substring(0, gnrNames.length - 2);
